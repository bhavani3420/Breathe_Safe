  ...
  try {
    ...
    try {
      ...
      const responseText = response.text();
        ...
        // Normalize and sanitize to avoid invalid JSON due to unescaped newlines or smart quotes
        ...
        // Helper: attempt to balance brackets/braces for truncated AI responses
        const balanceJsonBrackets = (text) => {
          ...
          // Close any remaining open containers in reverse order
          while (stack.length) {
        ...
        try {
          ...
          // Retry with balanced brackets/braces
          const repaired = balanceJsonBrackets(cleanedText);
          try {
            reportData = JSON.parse(repaired);
          } catch (repairedError) {
            console.error('Repaired JSON parse error:', repairedError);
            console.error('Repaired text:', repaired);
        ...
        // Normalize riskLevel values to match schema enum ['Low','Moderate','High','Severe']
        if (reportData.chronicDiseaseAnalysis && Array.isArray(reportData.chronicDiseaseAnalysis)) {
          const mapRisk = (val) => {
            if (!val || typeof val !== 'string') return 'Moderate';
            const s = val.trim().toLowerCase();
            if (s === 'low' || s === 'moderate' || s === 'high' || s === 'severe') return val.charAt(0).toUpperCase() + val.slice(1).toLowerCase();
            if (s === 'very high' || s === 'critical' || s === 'extreme' || s === 'urgent') return 'Severe';
            if (s === 'med' || s === 'medium' || s === 'mod') return 'Moderate';
            if (s === 'min' || s === 'minimal') return 'Low';
            return 'Moderate';
          };
          reportData.chronicDiseaseAnalysis = reportData.chronicDiseaseAnalysis.map(item => ({
            ...item,
